...
async function transferTokens() {
  ...
  const lookupTableAddress = process.env.LOOKUP_TABLE_ADDRESS;
  ...
  if (!lookupTableAddress) {
    throw new Error("LOOKUP_TABLE_ADDRESS environment variable is required");
  }
  ...
  try {
    ...
    // Create the main validateExecution instruction
    const validateExecutionIx = await saProgram.methods
      .validateExecution(
        {
      ...
      .signers([payerInfo.keyObject, mandatorySignerInfo.keyObject])
      .instruction();

    // Create the secp256r1 instruction
    ...
    // Create the vault executeBatch instruction
    const executeBatchIx = await vaultProgram.methods
      .executeBatch({
        deconstructedInstructions,
      })
      .accounts({
    ...
    // Collect all instructions in the correct order
    ...
    console.log("🔍 Lookup table account:", lookupTableAccount.value);
    console.log(
      "🔍 All instructions:",
      allInstructions.map((ix) => ix.keys)
    );

    // Create versioned transaction using TransactionMessage
    const blockhash = await provider.connection.getLatestBlockhash();

    // Create the transaction message with lookup table
    const transactionMessage = new TransactionMessage({
      payerKey: payerInfo.keyObject.publicKey,
      recentBlockhash: blockhash.blockhash,
      instructions: allInstructions,
      // addressLookupTables: [lookupTableAccount.value],
    } as any).compileToV0Message([lookupTableAccount.value]);

    // Create versioned transaction
    const versionedTx = new VersionedTransaction(transactionMessage);

    // Sign the transaction
    versionedTx.sign([payerInfo.keyObject, mandatorySignerInfo.keyObject]);
    ...
    console.log(`🔍 Using Address Lookup Table: ${lookupTableAddress}`);
