...
import { RAY_MINT, USDC_MINT, USDT_MINT } from "../../../tests/utils/consts";
import { getAssociatedTokenAddressSync } from "@solana/spl-token";
import { TOKEN_PROGRAM_ID } from "@solana/spl-token";
import { createUsdcUsdtDecreaseLiquidityIx } from "./raydium_v3_instructions";
...
async function decreaseLiquidityRaydium() {
  ...
  const rayTokenAccount = getAssociatedTokenAddressSync(
    RAY_MINT,
    executor.smartAccountHelper.vault,
    true, // allowOwnerOffCurve for PDA
    TOKEN_PROGRAM_ID
  );
  const depositInstruction = await createUsdcUsdtDecreaseLiquidityIx(
    executor.provider,
    executor.smartAccountHelper.vault,
...
async function main() {
  try {
    await decreaseLiquidityRaydium();
  } catch (error: any) {
