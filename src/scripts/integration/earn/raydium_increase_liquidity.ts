...
import { createUsdcUsdtIncreaseLiquidityIx } from "./raydium_v3_instructions";
...
async function increaseLiquidityRaydium() {
  ...
  const depositInstruction = await createUsdcUsdtIncreaseLiquidityIx(
    executor.provider,
    executor.smartAccountHelper.vault,
...
async function main() {
  try {
    await increaseLiquidityRaydium();
  } catch (error: any) {
